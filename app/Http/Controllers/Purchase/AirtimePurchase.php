            ...
            if ($check->count() == 1) {
                ...
                if ($this->core()->allow_pin == 1) {
                    if (trim($d_token->pin) == trim($request->pin)) {
                        ...
                        file_put_contents('debug_trace.txt', "Step 2: Auth Success (PIN verified)\n", FILE_APPEND);
                    } else {
                        Log::warning("PIN Validation Failed for user: {$d_token->username}. Sent: [" . ($request->pin ?? 'NULL') . "], Stored: [" . ($d_token->pin ?? 'NULL') . "]");
                    ...
                    $accessToken = $d_token->apikey;
                    file_put_contents('debug_trace.txt', "Step 2: Auth Success (PIN bypassed by setting)\n", FILE_APPEND);
                }
            } else {
                $accessToken = 'null';
            }
